...
export type CoursePhase = 'phase1_general' | 'phase2_department'
...
export interface TrainingRecord {
  ...
  created_at: string
  updated_at: string
  course?: Pick<Course, 'id' | 'course_code' | 'course_name' | 'category' | 'phase' | 'instructor'> | null
}
...
export const TRAINING_STATUS_OPTIONS: { value: string; label: string }[] = [
  { value: 'onboarding', label: 'Onboarding 報到中' },
  { value: 'phase1_general', label: 'Phase 1 · General 通識訓練' },
  { value: 'phase2_department', label: 'Phase 2 · Department 部門訓練' },
  { value: 'final_assessment', label: 'Final assessment 期末評核' },
  { value: 'graduated', label: 'Graduated 結業' },
  { value: 'transferred', label: 'Transferred 轉調' },
  { value: 'withdrawn', label: 'Withdrawn 退訓' },
]
...
async function apiFetch<T>(
  ...
  try {
    const response = await fetch(endpoint, {
      credentials: 'include', // Sends PHP session cookie (PHPSESSID)
      headers: {
    ...
    if (!response.ok || (body && body.ok === false)) {
      return {
        data: null,
        error: { message: body?.error ?? `Server returned ${response.status}` },
      }
    }
...
> {
  ...
  if (error || !data) {
    return { ok: false, error: error?.message ?? 'Failed to invite trainee' }
  }
...
export async function findOrCreateCourse(
  ...
  instructor?: string,
  category?: string,
): Promise<{ course_id: string; created: boolean; error: { message: string } | null }> {
  const cleanName = name.trim()
  if (!cleanName) {
    return { course_id: '', created: false, error: { message: 'Course name is required' } }
  }
  ...
  if (error || !data) {
    return { course_id: '', created: false, error: error ?? { message: 'Failed to find or create course' } }
  }

  return { course_id: data.course_id, created: data.created, error: null }
}
...
export async function getMyTraineeId(): Promise<{ trainee_id: string; error: { message: string } | null }> {
  ...
  return { trainee_id: data.trainee_id, error: null }
}
...
export async function listMyTrainingRecords(): Promise<{
  ...
  const { data, error } = await apiFetch<{ records: TrainingRecord[] }>('/matta/api/list_my_training_records.php')
  return { records: data?.records ?? [], error }
}

export async function listTraineeTrainingRecords(
  traineeId: string,
): Promise<{ records: TrainingRecord[]; error: { message: string } | null }> {
  const { data, error } = await apiFetch<{ records: TrainingRecord[] }>(
    `/api/list_trainee_training_records.php?trainee_id=${encodeURIComponent(traineeId)}`
  )
  return { records: data?.records ?? [], error }
}
...
export async function createMyTrainingRecord(
  ...
  return { record: data?.record ?? null, error }
}
...
export async function getMyTrainingProgress(): Promise<{
  ...
  const { records, error } = await listMyTrainingRecords()
  ...
  for (const r of records) {
    ...
    const phase = r.course?.phase
    if (phase === 'phase1_general') progress.phase1_hours += Number(r.hours)
    else if (phase === 'phase2_department') progress.phase2_hours += Number(r.hours)
  }

  return { progress, error: null }
}

export async function getTraineeTrainingProgress(traineeId: string): Promise<{
  progress: TrainingProgress
  error: { message: string } | null
}> {
